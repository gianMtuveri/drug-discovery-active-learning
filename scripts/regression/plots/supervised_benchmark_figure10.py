...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=Path(
            ...
            "multitarget/canonical_round_summary.csv"
        ),
        help=(
            "Canonical active-learning round summary "
            "used for Figure 10."
        ),
    )
    ...
    parser.add_argument(
        ...
        help="Directory for Figure 10 outputs.",
    )

    parser.add_argument(
        "--figure-name",
        default="figure10_supervised_vs_active_learning",
        help="Figure filename without extension.",
    )
...
def plot_supervised_vs_active_learning(
    *,
    target: str,
    models: list[str],
    active_learning_path: Path,
    cv_summary: pd.DataFrame,
    output_dir: Path,
    output_name: str,
) -> None:
    al = pd.read_csv(active_learning_path)
    ...
    missing = required.difference(al.columns)
    if missing:
        raise ValueError(
            "Active-learning summary is missing: "
            + ", ".join(sorted(missing))
        )

    al = al.loc[
        (al["target"].str.upper() == target.upper())
        & (al["model"].isin(models))
    ].copy()

    if al.empty:
        raise ValueError(
            f"No active-learning results found for target {target}."
        )

    initial_labelled = 20
    batch_size = 10
    al["n_labelled"] = initial_labelled + batch_size * al["round"]
    ...
    for ax, (metric, ylabel) in zip(axes, metrics, strict=True):
        for model in models:
            model_al = (
                al.loc[al["model"] == model]
                .sort_values("n_labelled")
            )
            ...
            x = model_al["n_labelled"].to_numpy(dtype=float)
            mean = model_al[f"{metric}_mean"].to_numpy(dtype=float)
            std = model_al[f"{metric}_std"].to_numpy(dtype=float)
            ...
            if cv_row.empty:
                raise ValueError(
                    f"No CV result found for model {model}."
                )

            cv_mean = float(
                cv_row.iloc[0][f"{metric}_mean"]
            )
    ...
    fig.suptitle(
        f"{target}: active learning versus full-data supervised performance",
        fontsize=14,
        fontweight="bold",
    )
...
def main() -> None:
    ...
    plot_supervised_vs_active_learning(
        target=args.target,
        models=args.models,
        active_learning_path=args.active_learning_summary,
        cv_summary=summary,
        output_dir=args.figure_dir,
        output_name=args.figure_name,
    )
